...
function App({ passcode, setPasscode }) {
  ...
  function addIncome(amount, note, walletId, date) {
    if (!amount || isNaN(amount)) return;
    const amt = Number(amount);
    setIncomeEntries(prev => [...prev, { id: uid(), date: date || todayStr(), amount: amt, note: note || "", walletId }]);
    setWallets(prev => prev.map(w => w.id === walletId ? { ...w, balance: Number(w.balance) + amt } : w));
  }
  function deleteIncome(entry) {
    setIncomeEntries(prev => prev.filter(e => e.id !== entry.id));
    setWallets(prev => prev.map(w => w.id === entry.walletId ? { ...w, balance: Number(w.balance) - Number(entry.amount) } : w));
  }
      ...
      <div style={styles.content}>
        {tab === "dashboard" && (
          <Dashboard upcoming={upcoming} wallets={wallets} onMarkPaid={markPaid} dailyConfig={dailyConfig}
            ...
            incomeEntries={incomeEntries} onDeleteIncome={deleteIncome} />
        )}
        {tab === "wallets" && (
  ...
  return (
    <div style={styles.tapeWrap}>
      <div style={styles.tape}>
        ...
        <div style={styles.tapeDivider} />
        {!showIncomeForm ? (
          <button style={styles.tapeAddIncomeBtn} onClick={() => { setIncomeDate(todayStr()); setShowIncomeForm(true); }}>
            <Plus size={14} /> Add income
          </button>
        ) : (
          <div style={styles.incomeFormStack}>
            <label style={styles.incomeDateLabel}>
              Date
              <input type="date" value={incomeDate} max={todayStr()} onChange={e => setIncomeDate(e.target.value)} style={styles.incomeInput} />
            </label>
            ...
            <div style={styles.incomeBtnRow}>
              <button style={styles.incomeConfirmBtn} onClick={() => { onAddIncome(incomeAmt, incomeNote, incomeWallet, incomeDate); setIncomeAmt(""); setIncomeNote(""); setShowIncomeForm(false); }}>
                <Check size={14} /> Add
              </button>
...
function Dashboard({ upcoming, wallets, onMarkPaid, dailyConfig, todaySpent, todayOver, monthlyBurnRate, forecast, incomeEntries, onDeleteIncome }) {
  const [payingId, setPayingId] = useState(null);
  const [payAmt, setPayAmt] = useState("");
  const [payWallet, setPayWallet] = useState(wallets[0]?.id || "");
  const [showIncomeList, setShowIncomeList] = useState(false);

  const recentIncome = [...incomeEntries].sort((a, b) => b.date.localeCompare(a.date) || (b.id > a.id ? 1 : -1)).slice(0, 15);
  ...
  return (
    <div>
      ...
      <button style={styles.incomeToggleLink} onClick={() => setShowIncomeList(s => !s)}>
        {showIncomeList ? "Hide" : "Review"} recent income ({incomeEntries.length}) <ChevronDown size={13} style={{ transform: showIncomeList ? "rotate(180deg)" : "none" }} />
      </button>
      {showIncomeList && (
        <div style={{ ...styles.list, marginBottom: 14 }}>
          {recentIncome.length === 0 && <EmptyState text="No income logged yet." />}
          {recentIncome.map(e => (
            <div key={e.id} style={styles.expenseEntryRow}>
              <span style={styles.dailyDate}>{e.date}</span>
              <span style={styles.expenseEntryNote}>{walletName(e.walletId)}{e.note ? ` · ${e.note}` : ""}</span>
              <span style={styles.expenseEntryAmount}>{fmtMoney(e.amount)}</span>
              <button style={styles.expenseDeleteBtn} onClick={() => onDeleteIncome(e)}><X size={13} /></button>
            </div>
          ))}
        </div>
      )}
...
const styles = {
  ...
  incomeDateLabel: { display: "flex", flexDirection: "column", gap: 4, fontSize: 11.5, color: "#B9C2BC" },
  incomeToggleLink: { background: "transparent", border: "none", color: "#24594A", fontSize: 12, display: "flex", alignItems: "center", gap: 4, padding: "0 0 12px", fontWeight: 500 },
